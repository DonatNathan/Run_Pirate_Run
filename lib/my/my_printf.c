...
void if_percentage(va_list list, char *s, int cmpt_char)
{
    ...
    if (s[cmpt_char] == 'S')
        octale_str(va_arg(list, char *));
    if (s[cmpt_char] == 'b')
        binary(va_arg(list, unsigned int));
    if (s[cmpt_char] == 'n')
        my_put_nbr(cmpt_char - 1);
    if (s[cmpt_char] == 'o')
        deci_to_octal(va_arg(list, int));
    if (s[cmpt_char] == 'u')
        integer(va_arg(list, int));
    if (s[cmpt_char] == 'x' || s[cmpt_char] == 'X')
        int_to_hexa(va_arg(list, int));
    if (s[cmpt_char] != 's' && s[cmpt_char] != 'i' && s[cmpt_char] != 'd' && s[cmpt_char] != 'c' && s[cmpt_char] != 'S' && s[cmpt_char] != 'b' && s[cmpt_char] != 'n' && s[cmpt_char] != 'o' && s[cmpt_char] != 'u' && s[cmpt_char] != 'x' && s[cmpt_char] != 'X')
